...
@section('panel')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body p-0">
                    <div class="table-responsive--md table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody>
                                @forelse($speeds as $key => $speed)
                                    <tr>
                                        <td data-label="@lang('Sl')">{{ $key + 1 }}</td>
                                        <td data-label="@lang('Name')">{{ __($speed->name) }}</td>
                                        <td data-label="@lang('Title')">{{ __($speed->title) }}</td>
                                        <td data-label="@lang('Price')">{{ getAmount($speed->price) }}
                                            {{ $general->cur_text }}</td>
                                        ...
                                        <td data-label="@lang('Action')">
                                            <button type="button" class="icon-btn edit" data-toggle="tooltip"
                                                data-id="{{ $speed->id }}" data-name="{{ $speed->name }}"
                                                data-title="{{ $speed->title }}"
                                                data-description="{{ $speed->description }}"
                                                data-image="{{ $speed->image }}"
                                                data-features="{{ json_encode(unserialize($speed->features)) }}"
                                                data-price="{{ getAmount($speed->price) }}" data-original-title="Edit">
                ...
                <div class="card-footer py-4">
                    {{ $speeds->links('admin.partials.paginate') }}
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- edit modal --}}
    <div id="edit-speed" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Edit speed')</h5>
                ...
                <form method="post" action="{{ route('admin.speed.update') }}" enctype="multipart/form-data">
                    @csrf
    ...
    <div id="add-speed" class="modal  fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add New speed')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form method="post" action="{{ route('admin.speed.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
...
@push('breadcrumb-plugins')
    <a href="javascript:void(0)" class="btn btn-sm btn--success add-speed"><i
