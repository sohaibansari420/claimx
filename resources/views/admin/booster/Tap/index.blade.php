...
@section('panel')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body p-0">
                    <div class="table-responsive--md table-responsive">
                        <table class="table table--light style--two">
                            ...
                            <tbody>
                                @forelse($taps as $key => $tap)
                                    <tr>
                                        <td data-label="@lang('Sl')">{{ $key + 1 }}</td>
                                        <td data-label="@lang('Name')">{{ __($tap->name) }}</td>
                                        <td data-label="@lang('Title')">{{ __($tap->title) }}</td>
                                        <td data-label="@lang('Price')">{{ getAmount($tap->price) }}
                                            {{ $general->cur_text }}</td>
                                        ...
                                        <td data-label="@lang('Action')">
                                            <button type="button" class="icon-btn edit" data-toggle="tooltip"
                                                data-id="{{ $tap->id }}" data-name="{{ $tap->name }}"
                                                data-title="{{ $tap->title }}"
                                                data-description="{{ $tap->description }}"
                                                data-image="{{ $tap->image }}"
                                                data-features="{{ json_encode(unserialize($tap->features)) }}"
                                                data-price="{{ getAmount($tap->price) }}" data-original-title="Edit">
                ...
                <div class="card-footer py-4">
                    {{ $taps->links('admin.partials.paginate') }}
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- edit modal --}}
    <div id="edit-tap" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Edit Booster Tap')</h5>
                ...
                <form method="post" action="{{ route('admin.tap.update') }}" enctype="multipart/form-data">
                    @csrf
    ...
    <div id="add-speed" class="modal  fade" tabindex="-1" role="dialog">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">@lang('Add New Booster Tap')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form method="post" action="{{ route('admin.tap.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
